...
public class UsersDao {
	...
	public boolean isExist(String inputId) {
		//아이디가 존재하는 지 여부를 알려주는 isExist 변수.
	...
	//로그인된 아이디의 정보들을 얻어낼 수 있는 메소드
	public UsersDto getData(String id) {
		//UsersDto 객체의 참조값을 담을 지역변수 만들기
		//로그인 된 회원의 정보를 담기위한 Dto
		UsersDto dto = null;
		//필요한 객체의 참조값을 담을 지역변수 만들기 
		Connection conn = null;
		PreparedStatement pstmt = null;
		...
		try {
			...
			//select 문 수행하고 결과 받아오기 
			rs = pstmt.executeQuery();
			//if문을 통해서 DB에 값이 존재하는 경우 회원 한명의 정보를 가져온다.
			if (rs.next()) {
				dto = new UsersDto();
				dto.setId(id);
	...
	public boolean isValid(UsersDto dto) {
		...
		//아이디 비밀번호가 유효한 정보인지 여부를 리턴한다.
		return isValid;
	}
	
	//회원 정보를 저장하는 메소드(profile 칼럼은 제외)
